---
title: "Verkefni1 - MAS201F"
author: "Isak Orn Ivarsson og Adalheidur Magnusdottir"
date: "2023-01-12"
output:
  html_document: default
  pdf_document: default
---
```{r setup, include= F}
knitr::opts_chunk$set(echo=T, warning=F, message =F)
```


Líkurnar á því að velja hurðina með bílnum sem þú vilt eru 1 á móti 3, þú velur því eina hurð með þeim líkum. Þá eru 2/3 líkur gegn þér þar sem það eru tvær hurðar eftir. Hinsvegar opnar Monty eina hurð þar sem geit er. Þá "þjappast" mótlíkurnar frá tveim hurðum yfir í eina. Líkurnar eru því ennþá 2/3 en núna bara á einni hurð. Það er því þér í hag að skipta um hurð þar sem hurðin sem Monty valdi ekki.

Hér má sjá Monty Hall gátuna setta stærðfræðilega fram, þar með talið  hvert útkomumengið er og hverjir eru atburðirnir sem vísað er til.

Útkomumengi:
$$
\Omega = \{Vinna, Tapa\}
$$
Atburðir:
Velja hurð = {A}

Monty Hall opnar hurð með geit = {B}


Tafla 1. Atburðir sem koma fyrir í þrautinni og líkurnar á þeim.

| Hurð 1        | Hurð 2          | Hurð 3        |
| :---          |    :----:       |  ---:         |
| Valin hurð    |  Óvalin hurð    | Óvalin hurð    |
| Líkur: 1 / 3  |  Líkur: 1/3     | Líkur: 1 / 3  |

Líkurnar eru því 1/3 þér í hag og 2/3 gegn þér. En þá opnar Monty Hall  eina hurð þar sem geit er.

| Hurð 1        | Hurð 2          | Hurð 3        |
| :---          |    :----:       |  ---:         |
| Valin hurð    | Monty Hall hurð | Óvalin hurð   |
| Líkur: 1 / 3  | Líkur: 0        | Líkur: 2 / 3  |

Þar sem Monty Hall opnar eina hurð þar sem staðfest að ein geit er þá eru líkurnar gegn þér ennþá 2/3 nema þú veist að það er geit á bakvið hurðina sem Monty opnaði og því "þjappast mótlíkurnar" á hurðina sem stendur eftir óopin, í þessu tilviki "Hurð 3".

Til að skýra betur frá því afhverju þátttakandi skuli alltaf skipta um hurð látum við P(A) vera atburðurinn að velja hurð 1 og P(B) vera atburðurinn að Monty Hall sýnir geit bakvið hurð 2.
$$
P(A|B) = \frac
  {P(B|A) P(A)} 
  {P(B)}
=
\frac
  {\frac{1}{2} * \frac{1}{3}}
  {\frac{1}{3} * \frac{1}{2} +
  \frac{1}{3} * 0 
  + \frac{1}{3} * 1}
=
\frac{1}{3}
$$

Í teljara setjum við 1/3 * 1/2 þar sem við veljum hurð og líkurnar á því að hún sé með bíl eru 1/3. Þá eru tvær hurðar eftir sem hafa helmings (1/2) líkur á því að vera valdar.
  Í nefnara setjum við líkurnar á Monty Hall að velji hurð 2.Hann velur aldrei hurð með bíl, þ.e. 1/3 * 0. Hann velur alltaf hurð sem hefur geit, þ.e. 1/3*1/2. 
Ef bíllinn er bakvið hurð 3 þá mun hann alltaf opna hurð 2, þ.e. 1/3*1
Þá getum við ályktað sem svo að líkurnar á því að velja hurð 1 og á bakvið hana sé bíll eru 1/3 en að velja hurð 3 hefur líkurnar 2/3. Því skal þátttakandi alltaf skipta um hurð. 


Ef að þáttastjórnandinn er búinn að steingleyma á bakvið hvaða hurð geitin er en tekur samt sénsinn og opnar eina hurð af handahófi og sem betur fer reynist geit á bakvið hurðina. Þá skiptir það ekki máli hvort þátttakandi skiptir um hurð eftir að þáttastjórnandinn opnar eina hurð.


Smíðuð var slembistærð sem lýsir hurðunum þremur og vinningnum á bak við þær. Geit er fimm þúsund króna virði en bíll er fimm milljóna króna virði.

Hurd = {5000, 5000, 5000000}

Hér er formúla líkindafalls slembistærðarinnar sem var búin til.

P(X=x)

Graf 1. Líkindafall slembistærðar.

Hermt var eftir 100000 þátttökum í leiknum og gróðinn reiknaður í hvert sinn sem ekki var skipt um hurð eftir að þátttastjórnandi opnar eina hurð með geit.

```{r}

hurd <- c('bill','geit','geit')
j <- 0
win2 <- 0
loss2 <- 0
utkoma <- 0

for(j in 1:100000){
  ValSpilara <- sample(hurd,1)
  
  if(ValSpilara=='bill'){
    utkoma <- utkoma + 5000000
    j+1
    win2 <- win2+1
  }
  if(ValSpilara =='geit'){
    utkoma <- utkoma + 5000
    j+1
    loss2 <-loss2+1
  }
  j <- j+1
}
```
af `r j` umferðum þá vinnur spilari í `r win2` skipti og endar með `r utkoma` gróða.

Hermt var einnig eftir 100000 þátttökum í leiknum og reiknaður gróðinn í hvert sinn þegar þátttakandi skiptir um hurð eftir að þátttastjórnandi hefur opnað eina hurð með geit.
```{r}
i <- 0
win <- 0
loss <- 0
SkiptiUtkoma <- 0

for(i in 1:100000){
  Spilari <- sample(hurd,1)
  Monty <- sample(hurd,1)
  
  while (Monty != 'geit'){
    Monty <- sample(hurd,1)
  }
  if(Monty == 'geit'){
    SpilariSkiptir <- sample(hurd,1)
  }
  while(SpilariSkiptir == Spilari){
    SpilariSkiptir <- sample(hurd,1)
  }
  if(SpilariSkiptir=='bill'){
    SkiptiUtkoma <- SkiptiUtkoma + 5000000
    win <- win+1
  }
  if(SpilariSkiptir=='geit'){
    SkiptiUtkoma <- SkiptiUtkoma + 5000
    loss <- loss +1
  }
  i <- i+1
}
```
af `r j` umferðum þá vinnur spilari í `r win` skipti og endar með `r SkiptiUtkoma` gróða.

5. Skrifið formúlu skilyrta líkindafallsins sem lýsir mögulegum útkomum þegar búið er að opna eina hurð með geit bakvið.



Væntigildi gróðans í leiknum eru líkurnar á því að vinna bíl margfaldað með verðmæti bílsins sem er svo samanlagt við líkurnar á því að vinna geit margfaldað með verðmæti geitarinnar.

Þegar þátttakandi skiptir ekki um hurð eftir að þátttastjórnandi hefur opnað eina hurð með geit er væntigildi gróðans: 

$$
E[X] = \frac{1}{3} * 5000000 + \frac{2}{3} * 5000
     = 1,669,999
$$

Væntigildi gróðans þegar einstaklingur skiptir um hurð eftir að þátttastjórnandi opnar eina hurð með geit er: 
$$
E[X] = \frac{1}{3} * 5000 + \frac{2}{3} * 5,000,000
     = 3,344,999
$$
Það má sjá að væntigildi gróðans er 3,444,999kr ef skipt er um hurð þegar þátttastjórnandi hefur opnað eina hurð en væntigildi er 1,669,999kr ef ekki er skipt um hurð. Það er því hagstæðara að skipta um hurð. 
